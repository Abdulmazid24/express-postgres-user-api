...
## ৪. সিকিউরিটি এবং অথেনটিকেশন (Security & Auth)

মডিউল-৮ এবং ৯-এ আমরা এপিআই-কে সিকিউর করার জন্য বেশ কিছু নতুন প্রযুক্তি যুক্ত করেছি:

### ৪.১ Password Hashing (bcryptjs)
আমরা ডাটাবেসে প্লেইন-টেক্সট (যেমন "12345") পাসওয়ার্ড সেভ করি না। `user.service.ts`-এ আমরা `bcryptjs` ব্যবহার করেছি:
```typescript
const hashPassword = await bcrypt.hash(password, 10);
```
*   **`bcrypt.hash(..., 10)`:** এটি পাসওয়ার্ডটিকে এনক্রিপ্ট করে একটি হিজিবিজি স্ট্রিং (যেমন `$2a$10$...`) বানিয়ে ফেলে। `10` হলো "Salt Round"। এর মানে হলো এটি পাসওয়ার্ডটিকে ১০ বার হ্যাশ করে, যা হ্যাকারদের Brute-Force অ্যাটাক করা অসম্ভব করে তোলে।

### ৪.২ JSON Web Token (JWT)
ইউজার যখন লগইন করে, তখন আমরা `jsonwebtoken` ব্যবহার করে তাকে একটি ডিজিটাল আইডি কার্ড (টোকেন) দিই।
*   **কেন JWT?** এটি স্টেটলেস (Stateless)। অর্থাৎ, ইউজার লগইন আছে কি না, তা চেক করার জন্য আমাদের ডাটাবেসে কুয়েরি চালাতে হয় না। টোকেনটি সার্ভারে আসলে সার্ভার তার `secret` কি (Key) দিয়ে টোকেনটি ভেরিফাই করলেই বুঝতে পারে ইউজারটি আসল কি না।

### ৪.৩ Custom Auth Middleware (`src/middleware/auth.ts`)
```typescript
const decoded = jwt.verify(token as string, config.secret as string) as JwtPayload;
req.user = decoded;
next();
```
*   **কী কাজ করছে?** এটি একটি পাহারাদারের মতো। যখন কোনো রাউটকে আমরা প্রটেক্ট করতে চাই (যেমন `router.get('/', auth(USER_ROLE.admin), userController...)`), তখন রিকোয়েস্টটি প্রথমে এই মিডলওয়্যারে আসে।
*   সে প্রথমে চেক করে টোকেন আছে কি না, তারপর টোকেন ভেরিফাই করে।
*   এরপর সে ডাটাবেসে চেক করে ইউজার এখনো অ্যাক্টিভ আছে কি না (`!user.is_active`)।
*   **Role-Based Access Control (RBAC):** সবশেষে সে চেক করে যে ইউজারের Role (যেমন admin বা user) এই রাউটে ঢোকার অনুমতি রাখে কি না। যদি সব ঠিক থাকে, তবে সে `next()` কল করে রিকোয়েস্টটিকে কন্ট্রোলারের কাছে যেতে দেয়।

### ৪.৪ Refresh Token & Cookies
সিকিউরিটি বাড়ানোর জন্য অ্যাক্সেস টোকেনের মেয়াদ কম দেওয়া হয় (যেমন ১ ঘণ্টা)। মেয়াদ শেষ হলে যেন ইউজারকে বারবার লগইন করতে না হয়, সেজন্য একটি **Refresh Token** জেনারেট করা হয় (মেয়াদ ৩০ দিন)। 
*   **HttpOnly Cookies:** রিফ্রেশ টোকেনটি আমরা সরাসরি রেসপন্সে না পাঠিয়ে `res.cookie(...)` এর মাধ্যমে ক্লায়েন্টের ব্রাউজারে `HttpOnly` কুকি হিসেবে সেভ করে দিই, যেন কোনো হ্যাকার জাভাস্ক্রিপ্ট দিয়ে সেটি চুরি করতে না পারে।

### ৪.৫ গ্লোবাল এরর হ্যান্ডলিং ও স্ট্যান্ডার্ড রেসপন্স
*   **`globalErrorHandler`:** অ্যাপ্লিকেশনের যেকোনো জায়গায় কোনো এরর হলে সেটি সরাসরি ক্লায়েন্টের কাছে না গিয়ে এই গ্লোবাল মিডলওয়্যারে চলে আসে। ফলে সব এররের ফরম্যাট একই রকম থাকে।
*   **`sendResponse` ইউটিলিটি:** প্রতিটি API রেসপন্স যেন হুবহু একই স্ট্রাকচারের (`success, message, data`) হয়, সেজন্য আমরা একটি `sendResponse` ফাংশন বানিয়েছি। এতে কোডের ডুপ্লিকেশন অনেক কমে যায়।

### ৪.৬ Logger Middleware
```typescript
app.use(logger);
```
এটি একটি গ্লোবাল মিডলওয়্যার। সার্ভারে যত রিকোয়েস্টই আসুক না কেন, এটি রিকোয়েস্টের মেথড (GET/POST) এবং টাইমস্ট্যাম্প ধরে একটি `logger.txt` ফাইলে সেভ করে রাখে। প্রোডাকশন সার্ভারে কোন ইউজার কখন হিট করছে, তার ট্র্যাকিং রাখার জন্য এটি খুবই জরুরি।
